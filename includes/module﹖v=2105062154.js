var statusElement = document.getElementById("status"),
    ...
    loadingLogoElement = document.getElementById("loadingLogo"),
    Module = {
        preRun: [],
        postRun: function() { mainContentElement.style.display = "block", loadingLogoElement.style.display = "none", document.getElementById("buttonShowHelp").className += " buttonFlash", window.dispatchEvent(new Event("mainContentVisible")) },
        print: function() {
            var e = document.getElementById("output");
            return e && (e.value = ""),
                function(t) { arguments.length > 1 && (t = Array.prototype.slice.call(arguments).join(" ")), console.log(t), e && (e.value += t + "\n", e.scrollTop = e.scrollHeight) }
        }(),
        printErr: function(e) { arguments.length > 1 && (e = Array.prototype.slice.call(arguments).join(" ")), console.error(e) },
...
Module.setStatus("Downloading..."), window.onerror = function() { Module.setStatus("不行就更换浏览器！"), spinnerElement.style.display = "none", Module.setStatus = function(e) { e && Module.printErr("[post-exception status] " + e) } };